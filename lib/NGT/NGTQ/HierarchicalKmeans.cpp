...
#ifdef NGTQ_QBG
void QBG::HierarchicalKmeans::treeBasedTopdownClustering(std::string prefix, QBG::Index &index,
                                                         uint32_t rootID, std::vector<float> &object,
  ...
  for (size_t id = 1; id <= numOfObjects; id++) {
    if (id % (numOfObjects / 100) == 0) {
      timer.stop();
      std::cerr << "# of processed objects=" << id << " " << id * 100 / numOfObjects << "% " << timer
                << " # of leaves=" << nleaves << std::endl;
      timer.start();
    }
    batch.push_back(id);
  ...
  hierarchicalKmeansBatch(batch, exceededLeaves, rootID, object, objectList, objectSpace, nodes, clustering,
                          maxSize, nleaves, 0);
...
void QBG::HierarchicalKmeans::threeLayerClustering(std::string prefix, QBG::Index &index) {
  ...
  auto &objectSpace = quantizer.globalCodebookIndex.getObjectSpace();
  {
    ...
    std::cerr << "HiearchicalKmeans::clustering: # of clusters=" << numOfThirdClusters << ":"
              << index.getQuantizer().property.globalCentroidLimit << std::endl;
    ...
    if (numOfFirstClusters > numOfSecondClusters) {
      std::stringstream msg;
      msg << "# of the first clusters should be larger than or equal to # of the second clusters. "
          << numOfFirstClusters << ":" << numOfSecondClusters;
      NGTThrowException(msg);
    }
    if (numOfSecondClusters > numOfThirdClusters) {
      std::stringstream msg;
      msg << "# of the third clusters should be larger than or equal to # of the second clusters. "
          << numOfSecondClusters << ":" << numOfThirdClusters;
      NGTThrowException(msg);
    }
    if (numOfFirstClusters > numOfSecondClusters) {
      std::stringstream msg;
      msg << "# of the second clusters should be larger than # of the first clusters. " << numOfFirstClusters
          << ":" << numOfSecondClusters;
      NGTThrowException(msg);
    }
    ...
    std::cerr << "clustering for the first... (" << vectors.size() << "->" << numOfFirstClusters << ") "
              << std::endl;
    ...
    timer.stop();
    std::cerr << "end of clustering for the first. # of clusters=" << firstClusters.size()
              << " time=" << timer << std::endl;
    ...
    timer.stop();
    std::cerr << "end of assign for the second. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;

    std::cerr << "subclustering for the second (" << numOfSecondClusters << ")..." << std::endl;
    std::vector<NGT::Clustering::Cluster> secondClusters;
    timer.start();
    subclustering(firstClusters, numOfSecondClusters, numOfSecondObjects, objectSpace, objectList, initMode,
                  secondClusters);
    timer.stop();
    std::cerr << "end of subclustering for the second. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;
    timer.start();
    NGT::Clustering::clearMembers(secondClusters);
    std::cerr << "assign for the third. (" << 1 << "->" << numOfThirdObjects << ")..." << std::endl;
    assignWithNGT(secondClusters, 1, numOfThirdObjects, objectSpace, objectList, epsilonExplorationSize,
                  expectedRecall);
    {
    ...
    timer.stop();
    std::cerr << "end of assign for the third. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;
    ...
    timer.start();
    subclustering(secondClusters, numOfThirdClusters, numOfThirdObjects, objectSpace, objectList, initMode,
                  thirdClusters);
    timer.stop();
    std::cerr << "end of subclustering for the third. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;
    ...
    assignWithNGT(thirdFlatClusters, 1, numOfObjects, objectSpace, objectList, epsilonExplorationSize,
                  expectedRecall);
    timer.stop();
    std::cerr << "end of assign all for the third. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;
...
void QBG::HierarchicalKmeans::twoPlusLayerClustering(std::string prefix, QBG::Index &index) {
  ...
  auto &objectSpace = quantizer.globalCodebookIndex.getObjectSpace();
  {
    ...
    std::cerr << "Two layer clustering. " << numOfFirstClusters << ":" << numOfFirstObjects << ","
              << numOfSecondClusters << ":" << numOfSecondObjects << "," << numOfThirdClusters << ":"
              << numOfThirdObjects << " " << numOfObjects << std::endl;
    ...
    std::cerr << "loading objects time=" << timer << " vmsize=" << NGT::Common::getProcessVmSize()
              << std::endl;
    ...
    firstClustering.kmeans(vectors, numOfFirstClusters, firstClusters);
    timer.stop();
    std::cerr << "kmeans clustering. # of clusters=" << firstClusters.size() << " time=" << timer
              << " vmsize=" << NGT::Common::getProcessVmSize() << std::endl;
    timer.start();
    std::cerr << "assign for the third (" << numOfFirstObjects << "-" << numOfThirdObjects << ")..."
              << std::endl;
    assign(firstClusters, numOfFirstObjects + 1, numOfThirdObjects, objectSpace, objectList);
    timer.stop();
    std::cerr << "assign for the third. time=" << timer << " vmsize=" << NGT::Common::getProcessVmSize()
              << std::endl;

    std::cerr << "subclustering for the third (" << numOfThirdClusters << ", " << numOfThirdObjects << ")..."
              << std::endl;
    std::vector<NGT::Clustering::Cluster> thirdFlatClusters;
    timer.start();
    subclustering(firstClusters, numOfThirdClusters, numOfThirdObjects, objectSpace, objectList, initMode,
                  thirdFlatClusters);
    timer.stop();
    std::cerr << "subclustering for the third. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;

    std::cerr << "assign all for the third (" << numOfThirdObjects << "-" << numOfObjects << ")..."
              << std::endl;
    timer.start();
    assignWithNGT(thirdFlatClusters, numOfThirdObjects + 1, numOfObjects, objectSpace, objectList);
    timer.stop();
    std::cerr << "assign all for the third. time=" << timer
              << ", vmsize=" << NGT::Common::getProcessVmPeakStr() << std::endl;

    std::vector<NGT::Clustering::Cluster> secondClusters;

    std::cerr << "clustering for the second. # of objects=" << thirdFlatClusters.size()
              << " # of clusters=" << numOfSecondClusters << std::endl;
    timer.start();
    vectors.clear();
    ...
    timer.stop();
    std::cerr << "clustering for the second. time=" << timer << " vmsize=" << NGT::Common::getProcessVmSize()
              << std::endl;
    NGT::Clustering::saveClusters(prefix + QBG::Index::getSecondCentroidSuffix(), secondClusters);
    ...
    for (size_t sidx = 0; sidx < secondClusters.size(); sidx++) {
      for (auto mit = secondClusters[sidx].members.begin(); mit != secondClusters[sidx].members.end();
           ++mit) {
    ...
    timer.stop();
    std::cerr << "permuting cluster time=" << timer << " vmsize=" << NGT::Common::getProcessVmSize()
              << std::endl;

    std::vector<size_t> cindex(numOfObjects);
    for (size_t cidx = 0; cidx < thirdPermutedClusters.size(); cidx++) {
      for (auto mit = thirdPermutedClusters[cidx].members.begin();
           mit != thirdPermutedClusters[cidx].members.end(); ++mit) {
        size_t vid  = (*mit).vectorID;
        cindex[vid] = cidx;
      }
    }
...
void QBG::HierarchicalKmeans::multiLayerClustering(QBG::Index &index, std::string prefix,
  ...
  if (prefix.empty()) {
    ...
    if (numOfRandomObjects > 0) {
      ...
      if (extractCentroid) {
        extractRandomObjectsFromEachBlob(of, nodes, numOfObjects, numOfRandomObjects - 1, quantizer,
                                         extractCentroid);
      } else {
        extractRandomObjectsFromEachBlob(of, nodes, numOfObjects, numOfRandomObjects, quantizer,
                                         extractCentroid);
      }
    }
  }
...
void QBG::HierarchicalKmeans::clustering(std::string indexPath, std::string prefix,
                                         std::string objectIDsFile) {
  ...
  std::cerr << "The specified params=FC:" << numOfFirstClusters << ":FO:" << numOfFirstObjects
            << ",SC:" << numOfSecondClusters << ":SO:" << numOfSecondObjects << ",TC:" << numOfThirdClusters
            << ":TO:" << numOfThirdObjects << ",O:" << numOfObjects << std::endl;
  ...
  if (clusteringType == QBG::HierarchicalKmeans::ClusteringTypeMultiLayer) {
    ...
    try {
      ...
      if (paddedDimension != dimension) {
        std::cerr << "HierarachicalKmeans: Warning! Dimensions are inconsistent. Dimension="
                  << paddedDimension << ":" << dimension << std::endl;
      }
...
void QBG::HierarchicalKmeans::assignAll(std::string indexPath, int64_t lowerBoundOfNoOfObjects,
                                        size_t noOfNearestNeighbors) {
  ...
  assignWithNGT(thirdFlatClusters, 1, numOfObjects, objectSpace, objectList, epsilonExplorationSize,
                expectedRecall, noOfNearestNeighbors);
  size_t remove = 0;
  size_t max    = 0;
