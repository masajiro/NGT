...
class Optimizer {
 public:
  ...
  static void catSubvector(vector<vector<float>> &vectors, vector<vector<float>> &subvectors) {
    if (vectors.size() == 0) {
  ...
  static double squareDistance(vector<vector<float>> &va, vector<vector<float>> &vb) {
  ...
  void evaluate(vector<vector<float>> &vectors, string &ofile, size_t &numberOfSubvectors,
                size_t &subvectorSize);
#endif

  void generateResidualObjects(vector<vector<float>> &globalCentroid, vector<vector<float>> &vectors) {
  ...
  void generateResidualObjects(string global, vector<vector<float>> &vectors) {
    if (global.empty()) {
  ...
  static void optimizeRotation(size_t iteration, vector<vector<float>> &vectors, Matrix<float> &xt,
...
#define ERROR_CALCULATION
  ...
  void optimize(vector<vector<float>> &vectors, vector<vector<float>> &globalCentroid, Matrix<float> &r,
                vector<vector<NGT::Clustering::Cluster>> &localClusters, vector<double> &errors);
  static size_t extractScaleAndOffset(const std::string indexPath, float clippingRate, int32_t nOfObjects,
                                      bool verbose);
