...
template <class JOB, class SHARED_DATA, class THREAD> class ThreadPool {
 public:
  ...
  class Thread : public THREAD {
   public:
    ...
    InputJobQueue &getInputJobQueue() { return threadPool->sharedData.inputJobs; }
    OutputJobQueue &getOutputJobQueue() { return threadPool->sharedData.outputJobs; }
